...
def plot_residual_stream_dynamics(
    ...
    """
    print("DEBUG residual_data TYPE:", type(residual_data))
    print("DEBUG residual_data TYPE:", type(residual_data))
print("DEBUG residual_data:", residual_data)

if isinstance(residual_data, list):
    print("DEBUG list length:", len(residual_data))
    if len(residual_data) > 0:
        print("DEBUG first item TYPE:", type(residual_data[0]))
        print("DEBUG first item:", residual_data[0])
    print("DEBUG residual_data:", residual_data)
...
def plot_hidden_state_pca(
    ...
    hidden_state : np.ndarray  [seq_len, d_model]
    layer_label  : e.g. "Layer 10"
    color_seed   : offsets the Viridis color scale for visual variety across layers
    """
    from sklearn.decomposition import PCA
...
def plot_top_logits_probabilities(
    ...
    Pipeline displayed in title:
        Hidden State → LM Head → Logits → Softmax → Probabilities

    Parameters
    ----------
    top_tokens : list[str]   decoded token strings, highest prob first
    top_probs  : list[float] corresponding probabilities (0‒1)
    top_k      : how many bars to draw
    """
