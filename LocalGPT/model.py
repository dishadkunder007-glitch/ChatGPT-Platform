"""
LocalGPT: Model & Mechanistic Interpretability Engine
...
class XRayModel:
    ...
    def inspect(self, text: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        # Convert hidden states to numpy array [num_layers, seq_len, d_model]
        hidden_states_np = np.array([h.numpy() for h in hidden_states])

        top_candidates = list(zip(top_tokens, top_probs))
    ...
    @torch.no_grad()
    def generate_step_by_step(self, prompt: str, max_new_tokens: int = 8, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        if self.is_synthetic or not current_ids:
            # Plausible autoregressive synthetic diagnostics
            sample_words = [" are", " neural", " network", " architectures", " designed", " for", " sequential", " data"]
            for step, w in enumerate(sample_words[:max_new_tokens]):
                tid = 1000 + step * 37
                current_ids.append(tid)
                conf = max(0.45, 0.92 - step * 0.06)
                ent = round(0.5 + step * 0.18, 3)
                top_cand = [(w, round(conf, 3)), (" model", round((1-conf)*0.4, 3)), (" system", round((1-conf)*0.3, 3)), (" based", round((1-conf)*0.2, 3)), (" which", round((1-conf)*0.1, 3))]
                history.append({
                    "step": step + 1,
                    "generated_token": w,
                    "generated_id": tid,
                    "full_text": prompt + "".join(sample_words[:step+1]),
    ...
    def generate_response(
        ...
        """
        Generates a natural language response to a user prompt or multi-turn message history,
        applying chat templates for instruct models and stopping at turn boundaries.
        """
        if self.is_synthetic:
        ...
        if isinstance(prompt, list):
            ...
            if self.tokenizer.hf_tokenizer is not None and getattr(self.tokenizer.hf_tokenizer, "chat_template", None):
                try:
                    formatted_text = self.tokenizer.hf_tokenizer.apply_chat_template(
                        prompt, tokenize=False, add_generation_prompt=True
                    )
                except Exception:
                    # Fallback to manual role formatting
                    formatted_text = self._format_messages_as_text(prompt)
            else:
                formatted_text = self._format_messages_as_text(prompt)
        else:
            # String input
            prompt_str = str(prompt).strip()
            if prompt_str.startswith("System:") or "\nUser:" in prompt_str:
                formatted_text = prompt_str
            elif self.tokenizer.hf_tokenizer is not None and getattr(self.tokenizer.hf_tokenizer, "chat_template", None):
                messages = [
        ...
        # Stop at subsequent turn boundaries if model generated extra turns
        for stop_marker in ["\nUser:", "\nHuman:", "\nAssistant:", "\nAI:"]:
    ...
    def _format_messages_as_text(self, messages: List[Dict[str, str]]) -> str:
        """Converts structured messages to plain text conversation."""
        lines = []
        for m in messages:
